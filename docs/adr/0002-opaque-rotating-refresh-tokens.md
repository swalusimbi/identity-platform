...
- **Family revocation on replay.** Redeeming a rotated token means two parties held the same token, which is theft by definition. Every refresh token the user has is revoked and both parties get signed out. The revocation reason is stored per row because the inference only holds for rotation: a token revoked by logout or self service session revocation gets presented again by the signed out device in normal operation, so those answer 401 without the family nuke
